...
> ✅ **依貴公司既有的 `docs/django_rules.md` 規範**：專案命名 `{platform}-{system}`、
...
> ⚠️ **P1 從 6–8 週調整為 8–10 週**：加入「資產清冊」（`09_資產與庫存管理.md` §11 方案 B）。
...
| 與現有會計軟體對帳不一致 | 中 | 中 | 已明確定義為單向匯出，本系統不做正式帳務（見 `00_網站目的與範圍.md` §6.3） |
...
`00_網站目的與範圍.md`｜`04_資料庫規劃.md`｜`05_API規格.md`｜`08_UI設計原則.md`
