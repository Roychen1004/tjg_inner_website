...
STAGE_TEMPLATES = [
    ...
    },
    {
        ...
        "stages": [
            ...
            # ★ 監造查驗＝土建的「簽收」。
            #
            # 決策 D14 說「監造查驗通過即可計價」，原本理解成「不需要簽收」——
            # 結果是土建工項永遠沒有 signoff_date，四種請款觸發方式
            # 全部靠 signoff_date 判斷，土建案的里程碑**永遠觸發不了**。
            #
            # 正確的理解：查驗通過**就是**業主方認可這件事完成了，
            # 跟鋼構的「業主簽收」是同一個概念，只是簽的人是監造。
            # 所以它該是 ✍＋💰 的站，跟鋼構的「進場簽收」對稱。
            (4, "inspect", "監造查驗", "#2a78d6",
             {"requires_signoff": True, "is_billing_trigger": True}, 14),
            (5, "done", "完成", "#104281", {}, None),
        ],
    },
]
