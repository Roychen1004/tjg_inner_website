...
公司就一間，天天用系統的是經營者與會計——**資料範圍不再按角色切**，
登入就看得到全部案子與進度。唯一的界線是錢：

  · 「檢視」角色拿到的 JSON 裡金額欄位是 null——值根本沒離開伺服器
  · 金流分頁與 API（應收/應付/現金流）整個不對「檢視」開放
  · 合約、發票類附件對「檢視」不可見也不可下載（欄位擋了、檔案沒擋，等於沒擋）
